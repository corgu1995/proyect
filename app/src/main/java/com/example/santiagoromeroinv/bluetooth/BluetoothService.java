...
public class BluetoothService {
    ...
    public static UUID UUID_SEGURO; //= UUID.fromString("computacionmovil.gr8.udea.edu.co.bluetoothexample.BluetoothService.Secure");
    public static UUID UUID_INSEGURO; //= UUID.fromString("computacionmovil.gr8.udea.edu.co.bluetoothexample.BluetoothService.Insecure");
    ...
    //Se crean los tres hilos necesarios para la comunicación
    private int          estado;
    ...
    public synchronized void iniciarServicio()
    {
        ...
        // Si existe una conexión previa, se cancela
        if(hiloConexion != null)
        {
    ...
    public synchronized void solicitarConexion(BluetoothDevice dispositivo)
    {
        ...
        // Comprobamos si existe un intento de conexión en curso.
        ...
        // Si existía una conexión abierta, se cierra y se inicia una nueva
        if(hiloConexion != null)
        {
            hiloConexion.cancelarConexion();
            hiloConexion = null;
        ...
        // Se instancia un nuevo hilo conector, encargado de solicitar una conexión
        // al servidor, que será la otra parte.
    ...
    // Sincroniza el objeto con el hilo Hilo Conexión e invoca a su método escribir()
    // para enviar el mensaje a través del flujo de salida del socket.
    ...
    // crear un hilo que maneje la conexión cuando ello ocurra.
    // La otra parte deberá solicitar la conexión mediante un HiloCliente.
    private class HiloServidor extends Thread
    {
        ...
        public HiloServidor()
        {
            ...
            // Creamos un socket para escuchar las peticiones de conexión
            try {
        ...
        public void run()
        {
            ...
            // El hilo se mantendrá en estado de espera ocupada aceptando conexiones
            // entrantes siempre y cuando no exista una conexión activa.
            // En el momento en el que entre una nueva conexión,
            while(estado != ESTADO_CONECTADO)
            {
                try {
                    // Cuando un cliente solicite la conexión se asigna valor al socket..
                ...
                // Si el socket tiene valor sera porque un cliente ha solicitado la conexión
                if(socket != null)
                {
                    ...
                    synchronized(BluetoothService.this)
                    {
                        switch(estado)
                        {
                            ...
                            case ESTADO_REALIZANDO_CONEXION:
                            {
                                ...
                                // Estado esperado, se crea el hilo de conexión que recibira
                                // y enviará los mensajes
                            ...
                            {
                                // No preparado o conexión ya realizada.
    ...
    // Hilo encargado de solicitar una conexión a un dispositivo que esté corriendo un
    // HiloServidor.
    private class HiloCliente extends Thread
    {
        ...
        public void run()
        {
            ...
            // Reiniciamos el hilo cliente, ya que no lo necesitaremos más
            synchronized(BluetoothService.this)
            {
                hiloCliente = null;
            }
    ...
    // Hilo encargado de mantener la conexión y realizar las lecturas y escrituras
    ...
    private class HiloConexion extends Thread
    {
        ...
        public HiloConexion(BluetoothSocket socket)
        {
            ...
            // no sería posible asignarles valor posteriormente si fallara esta llamada
        ...
        // Método principal del hilo, encargado de realizar las lecturas
        public void run()
        {
            debug("HiloConexion.run()", "Iniciando metodo");
            byte[] buffer = new byte[Constants.getDrawSize()];
            int bytes;
            setEstado(ESTADO_CONECTADO);
            // Mientras se mantenga la conexión el hilo se mantiene en espera ocupada
            ...
            while(true)
            {
                try {
                    ...
                    // Enviamos la información a la actividad a través del handler.
                    // El método handleMessage será el encargado de recibir el mensaje
